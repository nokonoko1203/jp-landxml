use jp_landxml::geometry::{calculate_tin_area, ClothoidCurve};
...
fn test_clothoid_point_calculation() {
    let start_point = Point2D { x: 0.0, y: 0.0 };
    let curve = ClothoidCurve::new(start_point, 0.0, 100.0, 50.0, None, Some(100.0));

    let point_at_start = curve
        .point_at_distance(0.0)
        .expect("Failed to calculate point");
    assert!((point_at_start.x - 0.0).abs() < 0.001);
    assert!((point_at_start.y - 0.0).abs() < 0.001);

    let point_at_mid = curve
        .point_at_distance(50.0)
        .expect("Failed to calculate point");
