...
pub fn export_debug_info(landxml: &LandXML) -> String {
    ...
    for (i, surface) in landxml.surfaces.iter().enumerate() {
        info.push_str(&format!("  Surface {}: {} (type: {:?})\n", i + 1, surface.name, surface.surface_type));
        info.push_str(&format!("    Points: {}, Faces: {}\n", 
