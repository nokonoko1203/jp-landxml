...
use jp_landxml::{
    CoordinateSystemValidator, HorizontalDatum, JLandXmlCoordinateSystem,
...
fn test_full_spec_coordinate_system_creation() {
    ...
    // 自動設定の確認
    assert_eq!(
        coord_system.plane_coordinate_zone,
        Some(JapanPlaneCoordinateSystem::Zone6)
    );
...
fn test_coordinate_system_validation() {
    ...
    assert_eq!(
        warnings.len(),
        0,
        "Should have no warnings for correct setup"
    );
...
fn test_validation_warning_display() {
    ...
    for warning in &warnings {
        ...
        assert!(
            ...
            "Warning display should not be empty"
        );
...
fn test_all_plane_coordinate_zones_with_complete_spec() {
    ...
    for zone_num in 1..=19 {
        ...
        .expect(&format!(
            "Should create coordinate system for zone {}",
            zone_num
        ));
        ...
        assert_eq!(
            coord_system.get_plane_coordinate_epsg(),
            Some(zone.epsg_code())
        );
