...
fn test_parse_real_sample_xml() {
    ...
    // データ量の検証（大まかな範囲）
    assert!(
        surface.definition.points.len() > 100000,
        "Expected large number of points"
    );
    assert!(
        surface.definition.faces.len() > 500000,
        "Expected large number of faces"
    );

    println!(
        "Successfully parsed {} points and {} faces",
...
fn test_sample_xml_point_validation() {
    ...
    for point in points.iter().take(100) {
        // 最初の100点だけチェック（パフォーマンス考慮）
        assert!(
            point.x > 70000.0 && point.x < 100000.0,
            "X coordinate out of expected range"
        );
        assert!(
            point.y > -40000.0 && point.y < 0.0,
            "Y coordinate out of expected range"
        );
        assert!(
            point.z > 30.0 && point.z < 40.0,
            "Z coordinate out of expected range"
        );
    }
}
...
fn test_sample_xml_face_validation() {
    ...
    for face in faces.iter().take(100) {
        // 最初の100面だけチェック
...
fn test_sample_xml_memory_usage() {
    ...
    let points_memory =
        landxml.surfaces[0].definition.points.len() * std::mem::size_of::<jp_landxml::Point3D>();
    let faces_memory =
        landxml.surfaces[0].definition.faces.len() * std::mem::size_of::<jp_landxml::Face>();
